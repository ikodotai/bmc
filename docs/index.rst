...
`MinIO <https://min.io>`_, an Amazon Simple Storage Service (S3) compatible object storage. It has a useful `Python client library <https://github.com/minio/minio-py>`_ which unfortunately lacks administrative capabilities that the `mc` and `minio` comman line interfaces provide, such as adding users and hosts, which we need to do for the `iko <https://iko.ai>`_ machine learning platform. This library solves that `problem <https://github.com/minio/minio-py/issues/829>`_.
